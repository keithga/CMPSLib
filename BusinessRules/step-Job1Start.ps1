...
foreach ( $CollectionName in Get-CMCollection -CollectionType Device -Name (Format-WAASPreAssessGroup -season '*' -group '*') | % Name -whatif:$False ) { 
    Get-CMDevice -CollectionName $CollectionName | 
        Move-CMDeviceToCollection -CollectionName $CollectionName -DestCollectionPostFix 'Preassessment'
}
