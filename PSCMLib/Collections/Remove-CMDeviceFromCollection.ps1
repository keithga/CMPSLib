...
Function Remove-CMDeviceFromCollection {
    ...
    .DESCRIPTION
    Uses WMI to remove many Direct Membership rules to a SCCM Device Collection with just one DB action
    .EXAMPLE
    Remove-CMDeviceFromCollection -CollectionID XXX00750 -System $ReadyForMigrationDevices[50..70] -passthru
    .NOTES
    ...
    process {
        foreach ( $sys in $System ) {
            if ( $sys.resourceID -in $FOundList ) {
