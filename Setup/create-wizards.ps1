...
foreach ( $Target in $Targets ) {
    ...
    if ($Path) {
        New-BatchSCriptWrapper -Command "(request-CMMoveToDay -SourceCollection '$(Format-WAASScheduling -Season $Season -Group $Target)' -Path ('$Path\' + [GUID]::newGuid().GUID + '\MoveToDay') )" -BatchScript "$PSscriptRoot\..\BusinessActions\Import_Ready_For_Scheduling_$Target.cmd"
        New-BatchSCriptWrapper -Command "(request-CMMoveToDay -SourceCollection '$(Format-WAASScheduling -Season $Season -Group $Target)' -Path ('$Path\' + [GUID]::newGuid().GUID + '\MoveToDay') -StripeCollection '$($Target)_OSD_W10_*' )" -BatchScript "$PSscriptRoot\..\BusinessActions\Import_Ready_For_Scheduling_With_Stripes_$Target.cmd"
    }
    else {
